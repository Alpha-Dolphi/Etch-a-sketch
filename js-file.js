...
let brushColor = "black",
  ...
  erase = false,
  first = 0,
  prevBackColor = `rgb(${hexToRgb("#FFFFFF").r}, ${hexToRgb("#FFFFFF").g}, ${
    hexToRgb("#FFFFFF").b
  })`;
if (first === 0) {
  first++;
...
function filling(value, backColor) {
  ...
  for (var i = 0; i < value * value; i++) {
    // Filling the fragment with div elements
...
function changeBackground(back) {
  ...
  cells.forEach((cell) => {
    if (
      cell.style.backgroundColor == prevBackColor &&
      cell.style.value == "clean"
    ) {
  ...
  // alert(back);
  prevBackColor = `rgb(${hexToRgb(back).r}, ${hexToRgb(back).g}, ${
    hexToRgb(back).b
  })`;
  // alert(prevBackColor);
  // painting();
